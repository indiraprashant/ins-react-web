import React, { Fragment } from "react";
import BannerImage from '../../assets/images/committee/grievance-redressal-committee.jpg'
const GrievanceRedressalCommittee = () => {
    return (
        <Fragment>
            <div className="col position_relative somi">
                <img src={BannerImage} className='img_res' />
            </div>
            <div className="">
                <div className="container custom_spacing">
                    <h3 className="text-center subheadingM mgBtmZero textPinkColor titleWithOutBanner">Grievance Redressal Committee</h3>
                </div>
                <div className="container custom_spacing custom_spacing_top_zero">
                    <div className="row">
                        <div className="col-md-12  backgroundColorWhite">
                            <h6 className="text-center tableTitle">LIST OF MEMBERS &nbsp;(2022-23)</h6>
                            <div className="table-responsive">
                                <table className="table table-bordered">
                                    <tbody>
                                        <tr className="backgroundColorLightPink">
                                            <td>
                                                <strong>Sr. No.</strong>
                                            </td>
                                            <td>
                                                <strong>Names of Members</strong>
                                            </td>
                                            <td>
                                                <strong>Designation</strong>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                1.
                                            </td>
                                            <td>
                                                Dr. Tarita Shankar
                                            </td>
                                            <td>
                                                Chairperson
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                2.
                                            </td>
                                            <td>
                                                Mr. Sandeep Gaekwad
                                            </td>
                                            <td>
                                                Director,&nbsp;Admin &amp; HR  IGI
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                3.
                                            </td>
                                            <td>
                                                Mrs. Vijaya Joshi<br />
                                            </td>
                                            <td>
                                                Executive Principal  Indira National School
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                4.
                                            </td>
                                            <td>
                                                Mrs. Aarti Garampalli
                                            </td>
                                            <td>
                                                Principal, &nbsp;Indira National School
                                            </td>
                                        </tr>


                                        <tr>
                                            <td>5.</td>
                                            <td>Mrs. Uma Pradeep</td>
                                            <td>Principal (Afternoon Shift)</td>
                                        </tr>
                                        <tr>
                                            <td>
                                                6.
                                            </td>
                                            <td>
                                                Mrs. Aakanksha Bhangaonkar
                                            </td>
                                            <td>
                                                Vice Principal  High School
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                7.
                                            </td>
                                            <td>
                                                Mrs. Indrani Das
                                            </td>
                                            <td>
                                                Vice Principal &nbsp;Secondary School
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                8.
                                            </td>
                                            <td>
                                                Mrs. Jayashree Thorat
                                            </td>
                                            <td>
                                                Vice Principal  Middle School
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                9.
                                            </td>
                                            <td>
                                                Mrs. Savita Sonawane
                                            </td>
                                            <td>
                                                Vice Principal  Primary School (Morning Shift)
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                10.
                                            </td>
                                            <td>
                                                Ms. Vijaya Borhade
                                            </td>
                                            <td width="50%">
                                                Vice Principal  Primary School (Afternoon Shift)
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                11.
                                            </td>
                                            <td>
                                                Mrs. Vrushali Deshmukh<br />
                                            </td>
                                            <td>
                                                Associate Director  Administration
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                12.
                                            </td>
                                            <td>
                                                Mrs. Preeti Munje
                                            </td>
                                            <td>
                                                Deputy Director  IT
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                13.
                                            </td>
                                            <td>
                                                Ms. Mrunal Kanherkar
                                                (Head Girl)

                                            </td>
                                            <td>
                                                Grade XII Alpha (Science)
                                                (Morning Shift)

                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                14.
                                            </td>
                                            <td>
                                                Mst. Prabhav Bajpayi
                                                ( Head Boy)

                                            </td>
                                            <td>
                                                Grade VIII – Alpha
                                                (Afternoon Shift)

                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </Fragment>
    )
}
export default GrievanceRedressalCommittee;